 ...
 * the server will listen on `app.get('protocol')`, `app.get('host')` and
 * `app.get('port')`.
 *
...
app.listen = function(cb) {
  ...
  var protocol = self.get('protocol');
  if (!protocol) {
    // force to http if not set
    protocol = 'http';
  }

  if (protocol != 'http') {
    // only in this case (https or http/2) we have to retrieve certificates
    var options = {
      key: fs.readFileSync(self.get('sslKeyPath'), 'utf8'),
      certs: fs.readFileSync(self.get('sslCertPath'), 'utf8'),
    };

    server = require(protocol).createServer(options, this);
  } else {
    server = require(protocol).createServer(this);
  }
